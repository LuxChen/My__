...
def search_github_repositories(keyword):
    ...
    search_url = f"https://api.github.com/search/repositories?q={keyword}&sort=updated&order=desc&per_page={MAX_REPOS_PER_KEYWORD}"
    try:
        response = requests.get(search_url, headers=headers, timeout=15)
        response.raise_for_status()
        return response.json().get('items', [])
    except requests.exceptions.RequestException as e:
        print(f"[!] 搜索仓库时出错 ({keyword}): {e}")
        return []
    except json.JSONDecodeError:
        print(f"[!] 解析搜索结果时出错 ({keyword}).")
        return []
        ...
        if isinstance(contents, list):
            for item in contents:
                if item['type'] == 'file' and (
                    item['name'].lower().endswith(('.txt', '.md')) or
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(future_to_keyword):
            try: repos_to_check.extend(future.result())
